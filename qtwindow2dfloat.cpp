...
public:
  ...
  static ColorPixel NonpositiveMin()
  {
    return Zero;
  }

  static bool IsPositive(ColorPixel val)
  {
    return true;
  }

  static bool IsNonpositive(ColorPixel val)
  {
    return false;
  }

  static bool IsNegative(ColorPixel val)
  {
    return false;
  }

  static bool IsNonnegative(ColorPixel val)
  {
    return true;
  }
...
//  cWinZBuffer = NULL;
  ...
  m_rotation = 0;
  cShowCrosshair = true;
  color = Palette::Generate(Palette::PALETTE_RGB);
...
QtWindow2DFloat::QtWindow2DFloat( QGLFormat glf, QWidget *parent, const char *name)
...
void
QtWindow2DFloat::SetInputImage(ImageType * newImData)
{
  cImData = newImData;
  ...
  cIWModeMin  = IW_MINFloat;
  cIWModeMax  = IW_MAXFloat;

  cImageMode = IMG_VALFloat;
  ...
  if( m_isslicer )
    ...
    if( m_id == 0 )
      ...
      m_slicer->setMaximum(newImData->GetLargestPossibleRegion().GetSize()[2] - 1);
    ...
    if( m_id == 1 )
      ...
      m_slicer->setMaximum(newImData->GetLargestPossibleRegion().GetSize()[1] - 1);
    ...
    if( m_id == 2 )
      ...
      m_slicer->setMaximum(newImData->GetLargestPossibleRegion().GetSize()[0] - 1);
...
void
QtWindow2DFloat::ChangeId(int id)
{
  m_id = id;
  cWinOrientation = m_id;
  ...
  if( m_isslicer )
    ...
    if( m_id == 0 )
      ...
      m_slicer->setMaximum(cImData->GetLargestPossibleRegion().GetSize()[2] - 1);
      ChangeSlice(z);
      }
    ...
    if( m_id == 1 )
      ...
      m_slicer->setMaximum(cImData->GetLargestPossibleRegion().GetSize()[1] - 1);
      ChangeSlice(y);
      // m_slicer->setValue(y);
      }
    ...
    if( m_id == 2 )
      ...
      m_slicer->setMaximum(cImData->GetLargestPossibleRegion().GetSize()[0] - 1);
      ChangeSlice(x);
      // m_slicer->setValue(x);
      }
    }
...
void
QtWindow2DFloat::SetInputImage2(ImageType * newImData)
{
  if( !newImData )
    {
    return;
...
const QtWindow2DFloat::ImagePointer &
QtWindow2DFloat
::GetInputImage2(void) const
{
  return cImData;
}
...
::SetInputOverlay( OverlayType * newOverlayData )
{
  ...
  if( (!m_isImage1) || (cImData_size == newoverlay_size) )
    ...
    calculator->Compute();
    float m_min = calculator->GetMinimum();
    float m_max = calculator->GetMaximum();
...
const QtWindow2DFloat::OverlayType::Pointer &
QtWindow2DFloat::GetInputOverlay( void )
const
{
  return cOverlayData;
}
...
void
QtWindow2DFloat::ViewOverlayData( bool newViewOverlayData)
{
...
bool
QtWindow2DFloat::ViewOverlayData(void)
{
  return cViewOverlayData;
}

void
QtWindow2DFloat::ViewOverlayCallBack(
...
void
QtWindow2DFloat::OverlayOpacity(float newOverlayOpacity)
{
...
float
QtWindow2DFloat::OverlayOpacity(void)
{
  return cOverlayOpacity;
}

QtWindow2DFloat::ColorTableType
* QtWindow2DFloat::GetColorTable(void)
{
  return cColorTable.GetPointer();
}

void
QtWindow2DFloat::update()
{
  if( !cValidImData )
    {
    return;
  ...
  if( m_overlaymax <= 3 )
    ...
    color[1 * 255].r = 1 * 65533; color[1 * 255].g = 0; color[1 * 255].b = 0;
    color[2 * 255].r = 0;       color[2 * 255].g = 1 * 65533; color[2 * 255].b = 0;
    color[3 * 255].r = 0;       color[3 * 255].g = 0; color[3 * 255].b = 1 * 65533;
    }
      ...
      for( int j = cWinMinX; j <= cWinMaxX; j++ )
        ...
        if( cOverlayData->GetPixel(ind) > m_overlaymax )
          ...
          if( m_overlaymax > 3 )
            {
            tf = 255
              - (float)( ( (float)cOverlayData->GetPixel(ind)
                           - (float)m_overlaymin) / ( (float)m_overlaymax - (float)m_overlaymin) ) * 255;
            }
          else
            {
        ...
        if( m_isOverlay2 )
          ...
          if( m_overlaymax2 > 3 )
            {
            tf2 = 255
              - (float)( ( (float)cOverlayData2->GetPixel(ind)
                           - (float)m_overlaymin2) / ( (float)m_overlaymax2 - (float)m_overlaymin2) ) * 255;
            }
          else
            {
        ...
        if( k <= (cWinMaxY * m_alpha) / 100 )
          ...
          m_value.SetRed( (unsigned char)(color[(int)tf * 255].r / 256) );
          m_value.SetGreen( (unsigned char)(color[(int)tf * 255].g / 256) );
          m_value.SetBlue( (unsigned char)(color[(int)tf * 255].b / 256) );
          ...
          if( m_isOverlay2 )
            {
            LabelPixelType m_value;
            m_value.SetRed( (unsigned char)(color[(int)tf2 * 255].r / 256) );
            m_value.SetGreen( (unsigned char)(color[(int)tf2 * 255].g / 256) );
            m_value.SetBlue( (unsigned char)(color[(int)tf2 * 255].b / 256) );
            m_value.SetAlpha( (unsigned char)(255) );
...
void QtWindow2DFloat::size(int w, int h)
{
  this->update();
  this->paintGL();
}
...
void QtWindow2DFloat::resizeGL( int w, int h )
{
...
void QtWindow2DFloat::initializeGL()
{
...
/** Draw */
void QtWindow2DFloat::paintGL(void)
{
  glMatrixMode(GL_PROJECTION);
  glLoadIdentity();
...
void QtWindow2DFloat::DrawString()
{
  ...
  if( m_drawimagename && (m_imagename != "") )
    {
    OutputString(0, offset, m_imagename.toLatin1() );
    offset -= 14;
    }
  ...
  if( m_drawposition )
    {
    OutputString(0, offset, QString("Pos: [%1,%2,%3]").arg(ind[0]).arg(ind[1]).arg(ind[2]).toLatin1() );
    offset -= 14;
    }

  if( m_drawintensity )
    {
    OutputString(0, offset, QString("Value: %1 [%2|%3]").arg(cImData->GetPixel(ind) ).arg(m_imagemin).arg(
                   m_imagemax).toLatin1() );
    offset -= 14;
    if( m_isOverlay1 )
      {
      OutputString(0, offset,
                   QString("Overlay: %1 [%2|%3]").arg(cOverlayData->GetPixel(ind) ).arg(m_overlaymin).arg(
                     m_overlaymax).toLatin1() );
      offset -= 14;
      }

    if( m_isImage2 )
      {
      OutputString(0, offset, QString("Value2: %1 [%2|%3]").arg(cImData2->GetPixel(ind) ).arg(cIWMin2).arg(
                     cIWMax2).toLatin1() );
      offset -= 14;
      }
    }
  ...
  if( m_drawimageinfo && (m_imageinfotype != "") )
    {
    OutputString(0, offset,
                 QString("Size: %1x%2x%3").arg(m_imageinfosize[0]).arg(m_imageinfosize[1]).arg(
                   m_imageinfosize[2]).toLatin1() );
    offset -= 14;
    OutputString(0, offset,
                 QString("Pixdim: %1x%2x%3").arg(m_imageinfopixdim[0]).arg(m_imageinfopixdim[1]).arg(m_imageinfopixdim[
                                                                                                       2]).toLatin1() );
    offset -= 14;
    OutputString(0, offset, QString("Type: %1").arg(m_imageinfotype).toLatin1() );
    offset -= 14;
    }
}

void QtWindow2DFloat::SetImageName(QString imagename)
{
...
void QtWindow2DFloat::SetManualIntensityOn()
{
  m_manualintensity = true;
}

void QtWindow2DFloat::SetImageMinMax(float min, float max)
{
  m_imagemin = min;
  m_imagemax = max;
}

void QtWindow2DFloat::GetImageMinMax(float& min, float& max)
{
  min = m_imagemin;
  max = m_imagemax;
}

void QtWindow2DFloat::SetDisplayOverlayZero(bool flag)
{
  m_displayoverlayzero = flag;
}

void QtWindow2DFloat::SetOverlayMinMax(float min, float max)
{
  m_overlaymin = min;
  m_overlaymax = max;
}

void QtWindow2DFloat::SetOverlayMinMax2(float min, float max)
{
  m_overlaymin2 = min;
  m_overlaymax2 = max;
}

void QtWindow2DFloat::DrawImageName(bool flag)
{
  m_drawimagename = flag;
  updateGL();
}

void QtWindow2DFloat::DrawPosition(bool flag)
{
  m_drawposition = flag;
  updateGL();
}

void QtWindow2DFloat::DrawIntensity(bool flag)
{
  m_drawintensity = flag;
  updateGL();
}

void QtWindow2DFloat::SetImageInfo(int* size, float* pixdim, QString type)
{
  m_imageinfosize = new int[3];
  m_imageinfopixdim = new float[3];
  for( int i = 0; i < 3; i++ )
    {
...
void QtWindow2DFloat::DrawImageInfo(bool flag)
{
  m_drawimageinfo = flag;
  updateGL();
}

void QtWindow2DFloat::OutputString( float x, float y, const char *string)
{
  int length;
...
void QtWindow2DFloat::SetZoomFactor(float zoom)
{
  view_zoom = zoom;
  updateGL();
}

void QtWindow2DFloat::SetViewPosition(float posx, float posy)
{
  view_pos_x = posx;
  view_pos_y = posy;
...
void QtWindow2DFloat::mouseMoveEvent( QMouseEvent *event )
{
  ...
  if( m_currentbutton == 1 )
    ...
    if( event->buttons() && event->modifiers() == Qt::ShiftModifier )
      {
...
void QtWindow2DFloat::mousePressEvent( QMouseEvent *event )
{
  if( m_isImage1 || m_isImage2 || m_isOverlay1 || m_isOverlay2 )
    {

    if( ( event->button() & Qt::LeftButton ) && !( event->buttons() & Qt::ControlModifier ) )
      {
      QPoint pos = static_cast<QMouseEvent *>( event )->pos();
      start_x = (float)pos.x();
      start_y = (float)pos.y();
      ...
      // float shift_x = ((float)view_pos_x*view_zoom*cSpacing[0]) + (float)width()/2.0  - ((float)cWinDataSizeX*
      // (float)view_zoom*cSpacing[0])/2.0;
      // float shift_y = ((float)view_pos_y*view_zoom*cSpacing[1]) + (float)height()/2.0 - ((float)cWinDataSizeY*
      // (float)view_zoom*cSpacing[1])/2.0;

      float shift_x =
        ( (float)view_pos_x
          * view_zoom) + (float)width() / 2.0  - ( (float)cWinDataSizeX * (float)view_zoom * cSpacing[0]) / 2.0;
      float shift_y =
        ( (float)view_pos_y
          * view_zoom) + (float)height() / 2.0 - ( (float)cWinDataSizeY * (float)view_zoom * cSpacing[1]) / 2.0;
...
void QtWindow2DFloat::SetCrosshair(int x, int y)
{
  image_x = x;
  image_y = y;

// printf("enter SetCrosshair, m_id=%d, x=%d, y=%d\n", m_id, x,y);
  if( m_id == 0 )
    {
    cClickSelect[0] = x;
    cClickSelect[1] = y;
...
void QtWindow2DFloat::GetCrosshair(int& x, int& y)
{
  x = image_x;
  y = image_y;
}

void QtWindow2DFloat::ChangeSlice(unsigned int value)
{
...
void QtWindow2DFloat::sliceNum(unsigned int newSliceNum)
{
...
unsigned int QtWindow2DFloat::sliceNum()
{
  return cWinCenter[cWinOrder[m_id]];
}

void QtWindow2DFloat::clickSelect(float newX, float newY, float newZ)
{
  // printf("enter clickSelect\n");
  cClickSelect[0] = newX;
  if( cClickSelect[0] < 0 )
    {
    cClickSelect[0] = 0;
...
void QtWindow2DFloat::SetIntensityMax(float value)
{
  cIWMax = value;
  update();
}

void QtWindow2DFloat::SetIntensityMin(float value)
{
...
void QtWindow2DFloat::SetManager(ManagerType* manager)
{
  m_manager = manager;
  manager->AddWindow2D(this);
}

void QtWindow2DFloat::SetSlider(QSlider* slicer)
{
  m_slicer = slicer;
  m_isslicer = true;
}

void QtWindow2DFloat::SetLabel(QLabel* label)
{
  m_label = label;
  m_islabel = true;
}

void QtWindow2DFloat::SetSliderValue(int value)
{
  if( m_isslicer )
    {
...
void QtWindow2DFloat::SetId(int id)
{
  m_id = id;
  cWinOrientation = m_id;
}

int QtWindow2DFloat::GetId()
{
  return m_id;
}

void QtWindow2DFloat::makeRasterFont(void)
{
  /*  GLuint i, j;
...
void QtWindow2DFloat::printString(char *s)
{
...
void QtWindow2DFloat::ChangeAlpha(int value)
{
  m_alpha = value;
  this->update();
}

void QtWindow2DFloat::ChangeLabelOverlay(int value)
{
...
void QtWindow2DFloat::SetRotation(float angle)
{
  m_rotation = angle;
  this->update();
}

void QtWindow2DFloat::ShowCrosshair(bool flag)
{
  cShowCrosshair = flag;
  this->update();
}

int QtWindow2DFloat::Bicubic_Interpol(unsigned char *m_image, int newx, int newy, int xsize, int ysize, float a,
                                      float b,
                                      int f)
{
...
float QtWindow2DFloat::CubicBSpline(float x)
{
...
float QtWindow2DFloat::CubicInterpolation(float x)
{
...
float QtWindow2DFloat::SquareFunction(float x)
{
...
float QtWindow2DFloat::TriangleFunction(float x)
{
...
float QtWindow2DFloat::CubicFunc(int f, float x)
{
...
float QtWindow2DFloat::BilinearFunc(int f, float x)
{
...
void QtWindow2DFloat::ResetView()
{
  view_pos_x = 0;
  view_pos_y = 0;
  view_zoom = 1;
...
bool QtWindow2DFloat::OverlayVisible()
{
  return m_overlayvisible;
}

void QtWindow2DFloat::HideOverlay()
{
...
void QtWindow2DFloat::ShowOverlay()
{
  if( m_isOverlay1 || m_isOverlay2 )
    {
    m_overlayvisible = true;
...
QtWindow2DFloat::SizeType QtWindow2DFloat::GetImageSize()
{
  return cImData->GetLargestPossibleRegion().GetSize();
}

QPixmap QtWindow2DFloat::RenderPixmap()
{
  makeCurrent();
  paintGL();
...
void QtWindow2DFloat::SetBlendingMode(int mode)
{
  m_blendingmode = mode;
  this->update();
  this->updateGL();
...
bool QtWindow2DFloat::IsCrosshairVisible()
{
  return m_crosshairvisible;
}

void QtWindow2DFloat::SetCrosshairVisible(bool flag)
{
