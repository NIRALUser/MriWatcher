...
QtWindow2DFloat::
QtWindow2DFloat( QGLFormat glf, QWidget *parent, const char *name)
...
void QtWindow2DFloat::DrawString()
{
  ...
  if (m_drawintensity)
  {
    OutputString(0,offset,QString("Value: %1 [%2|%3]").arg(cImData->GetPixel(ind)).arg(m_imagemin).arg(m_imagemax).toLatin1());
    offset -= 14;
    if (m_isOverlay1)
    {
      OutputString(0,offset,QString("Overlay: %1 [%2|%3]").arg(cOverlayData->GetPixel(ind)).arg(m_overlaymin).arg(m_overlaymax).toLatin1());
      offset -= 14;
    }

    if (m_isImage2)
    {
      OutputString(0,offset,QString("Value2: %1 [%2|%3]").arg(cImData2->GetPixel(ind)).arg(cIWMin2).arg(cIWMax2).toLatin1());
      offset -= 14;
    }
  }
 ...
 if (m_drawimageinfo && (m_imageinfotype != ""))
  {
    OutputString(0,offset,QString("Size: %1x%2x%3").arg(m_imageinfosize[0]).arg(m_imageinfosize[1]).arg(m_imageinfosize[2]).toLatin1());
    offset -= 14;
    OutputString(0,offset,QString("Pixdim: %1x%2x%3").arg(m_imageinfopixdim[0]).arg(m_imageinfopixdim[1]).arg(m_imageinfopixdim[2]).toLatin1());
    offset -= 14;
...
void QtWindow2DFloat::mousePressEvent( QMouseEvent *event ) 
{
   if (m_isImage1 || m_isImage2 || m_isOverlay1 || m_isOverlay2)
  {
    ...
    if(( event->button() & Qt::LeftButton ) && !( event->buttons() & Qt::ControlModifier ))
    {
     ...
     else if (( event->button() & Qt::MidButton ) || (event->modifiers()==Qt::ControlModifier && event->button()==Qt::LeftButton ))
     {
       QPoint pos = static_cast<QMouseEvent *>( event )->pos();
...
int QtWindow2DFloat::Bicubic_Interpol(unsigned char *m_image, int newx, int newy, int xsize, int ysize, float a, float b, int f)
{
