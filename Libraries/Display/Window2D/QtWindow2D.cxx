...
void
QtWindow2D::SetInputImage(ImageType * newImData)
{
  cImData = newImData;
...
void
QtWindow2D::SetInputImage2(ImageType * newImData)
{
  if( !newImData )
    {
    return;
...
void
QtWindow2D::ViewOverlayData( bool newViewOverlayData)
{
...
void QtWindow2D::DrawString()
{
  ...
  if( m_drawintensity )
    ...
    if( m_isOverlay1 )
      ...
      OutputString(0, offset,
                   QString("Overlay: %1 [%2|%3]").arg(cOverlayData->GetPixel(ind) ).arg(m_overlaymin).arg(m_overlaymax) );
      offset -= 14;
      }
  ...
  if( m_drawimageinfo && (m_imageinfotype != "") )
    {
    OutputString(0, offset,
                 QString("Size: %1x%2x%3").arg(m_imageinfosize[0]).arg(m_imageinfosize[1]).arg(m_imageinfosize[2]) );
    offset -= 14;
    OutputString(0, offset,
...
int QtWindow2D::Bicubic_Interpol(unsigned char *m_image, int newx, int newy, int xsize, int ysize, float a, float b,
                                 int f)
{
