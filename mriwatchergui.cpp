...
void MriWatcherGUI::LoadFile(const QString& filename)
{
    ...
    QString pixeltypestring(QString::fromStdString(reader->GetImageIO()->GetComponentTypeAsString(reader->GetImageIO()->GetComponentType())));
...
void MriWatcherGUI::ReDraw()
{
	...
	else
	{
	    ...
	    m_frame->setMinimumHeight((((m_numberofimages-1)/nb_column)+1)*(g_scrollArea->viewport()->size().width()/nb_column));
	    m_imageframelayout->setColumn(nb_column);
        }
    }
}
...
MriWatcherGUI::ImageType::Pointer MriWatcherGUI::CreateNewImage(int sizex,int sizey,int sizez,float dimx,float dimy,float dimz)
{
    int imagesize[3];
    imagesize[0] = sizex;
    imagesize[1] = sizey;
    ...
    switch (e->type())
    {
	    ...
	    if (static_cast<QKeyEvent *>( e )->key()== Qt::Key_Plus )
	    {
		    ...
		    if (m_imageframelist[i]->IsSelected())
		    {
			    ...
			    m_imageframelist[i]->g_imageframe->ChangeLabelOverlay((int)(m_imageframelist[i]->g_imageframe->cOverlayOpacity*100+5));
                    }
            }
